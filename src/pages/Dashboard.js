...
const DataTable = ({ data, tipeInsidenList, refresh, triggerDelete }) => {
  ...
  const handleChange = async (id, field, value) => {
    try {
      await axios.put(`http://localhost:5000/update`, { id, field, value });
      refresh();
    } catch {
...
const App = () => {
  ...
  const fetchGrafik = async () => {
    try {
      const res = await axios.get('http://localhost:5000/grafik');
            ...
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '12px', overflow: 'hidden' }}>
              ...
              <div className="modal-footer border-0 pt-0">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      await axios.delete(`http://localhost:5000/delete/${modalData.id}`);
                      fetchData();
